...
spiral:                                           
  #step 1: assign dimensions and corners to the registers
	addi $sp, $sp,         -4
	sw    $ra, 0($sp)	    #save ra to main in the stack
	add   $t0,  $zero,  $a0    #t0 is the address of the first element of dimensions
 	lw    $s0,  0($t0)	    #s0 = i (frame rows)
 	lw    $s1,  4($t0)	    #s1 = j (frame columns)
 	lw    $s2,  8($t0)             #s2 = k (window rows)
 	lw    $s3,  12($t0)           #s3 = l (window columns)
	add   $v0,  $zero,  $zero    #v0 = 0 (match address)
...
firstforLoop:                                   				     
     beq 	$t0, $t1, exit_1st_loop          # if t0 == 61, break loop 1
     add  	$t3,  $zero,  $zero   		# t3 = 0 (j for the 2nd for loop)
	secondforLoop:
     		beq $t3, $t1, exit_2nd_loop   	# if t3 == 61, breal loop 2 
...
Get_index: 
	add $t5,  $zero,  $v0                          #t5 = v0 (1d index)
	addi $t5, $t5, 1
	add $t7, $zero, $zero			# i = 0
	Workindex:
		#bge $s1, $t5, Skip                  # if s1 > t5, go to Skip
		sub $t6, $s1, $t5
		bgez $t6, Skip		

		sub $t5, $t5, $s1		# t5 = t5 - frameColumn
		addi $t7, $t7, 1		# i++
		j Workindex
	Skip:
		addi $t5, $t5, -1		# i++
		add $v0, $zero, $t7		# v0 match row
		add $v1, $zero, $t5		# v1 match column
...
SAD: 
	add $t6,   $zero,   $zero  		# t6 (sum) = 0
	add $t2,   $zero,   $zero       		# t2(i) = 0
      
	Sad_loop1:
		beq $s2,    $t2,     ifCondition         # if s2 == t2, break loop1
		add $t5,   $zero,      $zero 	        #t5(j) = 0

		Sad_loop2:
			beq $s3,     $t5,    exit_Sad_Loop2         # if s3 == t5, break loop2
			mul $t8,     $t2,     $s1                  # t8 = (i * framecolumns)
			add $t8,     $t8,     $t5                   # t8 = (i * framecolumns) + j
			add $t8,     $t8,     $a0                  # t8 = (i * framecolumns) + j + address
			sll    $t8,     $t8,       2                    # t8 = t8 * 4
			add $t7,     $a1,     $t8                  # t7 = &a1 + $t8 (address of eva. ele. of frame)
			lw    $t8,    0($t7)        		      # t8 = (content of eva. ele. of frame)       
			mul $t9,     $t2,      $s3         	      # t9 = (i * windowcolumns)
			add $t9,     $t9,      $t5 	      # t9 = (i * windowcolumns) + j
			sll   $t9,      $t9,        2        	      # t9 = t9 * 4
			add $t7,     $a2,     $t9	                  # t7=&a2 + $t9 (address of eva. ele. of window)
			lw    $t9,     0($t7)               	      # t7 = (content of eva. ele. of window)  
			sub $t7,      $t8,      $t9           	      # t7 = t8 - t9
			bgtz $t7,  correctSign        # if t7 is positive (0 <) go to correctSign
			addi $t9,     $zero,   -1
			mul $t7,      $t7,       $t9                   # t7 = t7 *( -1) 
		correctSign: 
			add $t6, $t6, $t7                # sum = sum + Abs_Val_of_Diff
			addi $t5,    $t5,        1 	  	      # j++
			j Sad_loop2
		exit_Sad_Loop2:
			addi $t2,   $t2,       1  	                 # i++
...
ifCondition:
	slt    $t7,      $t6,       $v1                # if sum < content, t7 = 1
	beq $t7,       $zero,   exitSAD	       # if t7 == 0, go to exitSAD
	add $v1,      $zero,   $t6                # content(minimum SAD) = sum
	add $v0,      $zero,   $a0               # match_address = address

exitSAD: 
	j $ra     #return to Spiral Function
#####################################################################
